...
def transform_crime_mental_health_dataframes(crime_df, mental_health_df):
    try:
        ...
        raise RuntimeError(f"Error transforming crime and mental health dataframes: {e}")
...
def remove_columns(crime_df, mental_health_df):
    try:
        ...
        raise RuntimeError(f"Error removing columns or converting data types: {e}")
...
def get_countries_to_years(df):
    try:
        ...
        raise RuntimeError(f"Error creating countries to years mapping: {e}")
